...
Skills 11–13 (cooking-mode commands) are implemented as backend stubs that emit `uiHint` events; full integration depends on the cooking-mode page (see [cooking-mode spec](../specs/cooking-mode.md), still planned). Skills 8–9 require a schema bump (`eaten_at` per meal slot). Skill 10 (`get_inflammation_index`) is heuristic-based for now because the per-ingredient nutrition catalog described in [nutrition spec](../specs/nutrition.md) is not fully populated yet — when it is, the heuristic gets replaced.
