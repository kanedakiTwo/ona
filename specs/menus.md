...
- Users can switch between **"Vista día"** and **"Vista semana"** via a toggle in the menu header. "Vista día" is the default (the existing day-by-day photo-card layout). "Vista semana" renders a **transposed grid** with days as rows and meal types as columns — meal-type columns whose week has zero recipes are hidden, so a household that doesn't plan desayuno/merienda only sees comida + cena columns. Meal headers use lucide icons (sunrise / sun / sunset / moon) rather than text labels to keep header height minimal. Each cell shows a thumbnail + a `shortRecipeName`-trimmed title (drops "Cómo hacer…", "Receta de…", "Las N recetas que…" SEO openers; truncates at 24 chars). Tapping any cell flips back to "Vista día" with that day selected. **Drag-and-drop**: each filled cell can be dragged onto any other cell to move (or swap) the slot — backed by `POST /menu/:menuId/move-slot` for atomicity. The choice between vista día/semana persists in `localStorage.ona.menu.view`
