...
- Logged-in users browse the catalog at `/recipes` as a 2-column grid of photo cards. Each card carries an ownership badge: **"ONA"** (system catalog, `authorId = null`) or **"Tuya"** (`authorId = user.id`). **Other users' recipes are never returned** — every authenticated caller (including admins) sees only system recipes + their own. A segmented control above the search bar filters the grid by scope: **Todas** (default), **Mis recetas**, **Catálogo ONA**. The choice persists in `localStorage.ona.recipes.scope`. When the user has copied a system recipe via "Añadir a mis recetas", the catalogue listing **suppresses the original** from this user's view (via the `copied_from_recipe_id` back-reference) so the same dish doesn't appear twice
